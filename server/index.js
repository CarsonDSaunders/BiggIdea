...
app.post('/api/authenticate', async (req, res) => {
    ...
    if (username === '' && password === '') {
        ...
        if (foundUser.length > 0) {
            ...
            bcrypt.compare(password, hash, async (err, authenticated) => {
...
app.put('/api/password/:id', async (req, res) => {
    ...
    bcrypt.hash(password, saltRounds, (err, hash) => {
        dbInstance.user_update_password(userId, hash).then(() => {
            res.sendStatus(200);
        });
    });
});
...
app.post(
    ...
    async (req, res) => {
        ...
        var params = {
            Bucket: 'biggidea',
            Body: file.buffer,
            Key: `avatars/${userId}/${file.originalname}`,
        };
...
app.delete('/api/boards/:id', requireLogin);
